...
export default function ResourceRequestForm({ resourceTitle }: { resourceTitle: string }) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (MAILCHIMP_URL) {
      try {
        ...
        formData.append('MMERGE6', org) // custom Mailchimp merge field for organization
        formData.append('MMERGE7', resourceTitle) // custom merge field to track which resource
  ...
  if (status === 'success') {
    return (
      <div className="rounded-2xl bg-emerald-50 border border-emerald-200 p-8 text-center">
        ...
        <h2 className="text-2xl font-bold text-primary-900 mb-2">On its way.</h2>
        <p className="text-gray-600">
          Check your inbox — <strong>{resourceTitle}</strong> should arrive within a few minutes.
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      <p className="text-xs text-gray-500 text-center">
        By submitting, you agree to receive occasional emails from Launchpad Solutions. Unsubscribe anytime.
      </p>
    </form>
  )
}
